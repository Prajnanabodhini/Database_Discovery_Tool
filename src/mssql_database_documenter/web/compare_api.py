...
from ..comparison import compare_run_paths, export_comparison, row_matches_status, summarize_rows
...
def comparison_rows(comparison_id: str):
    ...
    if status: rows = [row for row in rows if row_matches_status(row, status)]
    ...
    filtered_summary = summarize_rows(rows, scope="CATEGORY_FILTERED_ROWS", category=category)
    return jsonify({"category": category, "page": page, "per_page": per_page, "total": len(rows), "rows": rows[start:start + per_page], "summary": filtered_summary, "category_summary": result["categories"][category]["summary"], "global_summary": result["summary"]})
