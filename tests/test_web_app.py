...
class WebAppTests(unittest.TestCase):
    ...
    def test_report_regeneration_is_run_selected_offline_and_source_preserving(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            destination = Path(job.result["destination"]).resolve()
            self.assertNotIn(source.resolve(), destination.parents)
            relative = Path(job.result["html"]).resolve().relative_to((base / "output").resolve()).as_posix()
